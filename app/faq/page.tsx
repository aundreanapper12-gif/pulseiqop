...
export default function FAQPage() {
  return (
    <main className="min-h-screen bg-[#f4efe7] text-[#101010]">
      <header className="border-b border-black/10 px-5 py-4 md:px-8"><div className="mx-auto flex max-w-5xl items-center justify-between gap-4"><a href="/" className="flex items-center gap-3 font-black"><span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-black text-white"><Sparkles size={18} /></span><span>PulseIQ Operations</span></a><a href="/" className="inline-flex items-center gap-2 text-sm font-black text-black/55 hover:text-black"><ArrowLeft size={16} /> Home</a></div></header>
      <section className="px-5 py-16 md:px-8 md:py-24"><div className="mx-auto max-w-5xl">
        ...
        <p className="mt-8 text-sm font-black uppercase tracking-[0.22em] text-black/35">Client Questions</p>
        <h1 className="mt-3 text-5xl font-black tracking-tight md:text-7xl">Clear Answers Before You Make A Decision.</h1>
        <p className="mt-6 max-w-3xl text-lg leading-8 text-black/58">Understand what PulseIQ analyzes, how the free diagnostic differs from paid work, what information is appropriate to share, and what you can expect from each service level.</p>
        <div className="mt-12 space-y-4">{faqs.map(([question, answer]) => <details key={question} className="group rounded-[1.6rem] border border-black/10 bg-white/75 p-6 shadow-sm"><summary className="cursor-pointer list-none pr-8 text-xl font-black marker:hidden">{question}</summary><p className="mt-4 max-w-4xl leading-7 text-black/55">{answer}</p></details>)}</div>
        <div className="mt-12 rounded-[2rem] bg-black p-7 text-white md:flex md:items-center md:justify-between md:gap-8 md:p-9"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-white/40">Need A Recommendation?</p><h2 className="mt-2 text-3xl font-black">Start With The Business Decision, Not The Package.</h2><p className="mt-3 max-w-2xl leading-7 text-white/55">Choose “Help Me Choose” on the request page and describe the question you need answered. PulseIQ will help identify the smallest useful starting point.</p></div><a href="/request" className="mt-6 inline-flex rounded-full bg-white px-6 py-4 font-black text-black md:mt-0">Request An Analysis</a></div>
        <div className="mt-12 flex flex-wrap gap-3 border-t border-black/10 pt-8 text-sm font-black"><a href="/methodology" className="rounded-full border border-black/15 px-5 py-3 hover:bg-black hover:text-white">Methodology</a><a href="/privacy" className="rounded-full border border-black/15 px-5 py-3 hover:bg-black hover:text-white">Privacy</a><a href="/terms" className="rounded-full border border-black/15 px-5 py-3 hover:bg-black hover:text-white">Terms</a></div>
      </div></section>
    </main>
  );
}
