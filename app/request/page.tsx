...
export default function RequestPage() {
  ...
  const validate = () => { if (!requiredComplete || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.email.trim())) { setStatus("Enter your name, a valid business email, business name, and business question first."); return false; } return true; };
  ...
  const emailRequest = (event: FormEvent) => {
    ...
    track("request_email_draft", { service: data.service });
  ...
  const openCheckout = () => {
    ...
    track("checkout_started", { service: data.service });
    window.location.assign(checkoutUrl.toString());
    setStatus(`Continuing To Secure Checkout With Request Reference ${id}. Use The Same Business Email And Short Question So PulseIQ Can Match Your Payment And Request.`);
  };
  ...
  return (
    <main className="min-h-screen bg-[#f3f6fb] text-[#0f172a]">
      ...
      <section className="px-5 py-16 md:px-8 md:py-24"><div className="mx-auto max-w-6xl">
        ...
        <div className="mt-8 flex flex-wrap gap-3 text-sm"><span className="rounded-xl border border-slate-200 bg-white px-4 py-3">1. Choose your analysis</span><span className="rounded-xl border border-slate-200 bg-white px-4 py-3">2. Complete secure checkout</span><span className="rounded-xl border border-slate-200 bg-white px-4 py-3">3. Agree on data & delivery</span><a href="/next-steps" className="rounded-xl bg-blue-50 px-4 py-3 font-semibold text-blue-700">See the full delivery process →</a></div>
        <div className="mt-12 grid gap-6 lg:grid-cols-[.82fr_1.18fr]">
          ...
          <form onSubmit={emailRequest} className="rounded-2xl border border-slate-900/10 bg-white p-6 shadow-xl md:p-8">
            ...
            <p className="mb-5 text-sm leading-6 text-slate-600">Before purchasing, review our <a href="/terms#refunds" className="font-semibold text-blue-700 underline">refund and cancellation information</a> and <a href="/privacy" className="font-semibold text-blue-700 underline">privacy policy</a>.</p>
            ...
            {status ? <div role="status" aria-live="polite" className="mt-5 flex items-start gap-3 rounded-2xl border border-slate-900/10 bg-white p-4 text-sm font-semibold leading-6 text-slate-600"><CheckCircle2 className="mt-0.5 shrink-0" size={18} /> {status}</div> : null}
