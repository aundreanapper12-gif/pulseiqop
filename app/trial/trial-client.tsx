...
import { parseTrialState, trialStatus, type TrialState } from "./state";
...
export default function TrialClient() {
  ...
  const [existing, setExisting] = useState<TrialState | null>(null);
  const [nowMs, setNowMs] = useState(0);

  useEffect(() => {
    try { setExisting(parseTrialState(window.localStorage.getItem(TRIAL_KEY))); } catch {}
    setNowMs(Date.now());
  }, []);

  const startTrial = (event: FormEvent) => {
    event.preventDefault();
    if (existing && trialStatus(existing, Date.now()) === "active") {
      router.push("/dashboard");
      return;
    }
    if (existing && trialStatus(existing, Date.now()) === "expired") {
      router.push("/pricing");
      return;
    }
    ...
    <main className="min-h-screen bg-[#f3f6fb] px-5 py-12 text-[#0f172a] md:px-8 md:py-20">
      <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[.9fr_1.1fr] lg:items-start">
        ...
        <form onSubmit={startTrial} className="rounded-3xl border border-slate-900/10 bg-white p-6 shadow-xl md:p-8">
          ...
          <button type="submit" className="mt-7 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-blue-700 px-5 py-4 font-semibold text-white shadow-lg">{existing && nowMs ? (trialStatus(existing, nowMs) === "active" ? "Continue my PulseIQ trial" : "View plans") : "Start my 14-day free trial"} <ArrowRight size={18} /></button>
